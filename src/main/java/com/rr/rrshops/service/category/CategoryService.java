...
import com.rr.rrshops.expection.ResourceAlreadyExistesExeption;
...
public class CategoryService implements ICategoryService {
    ...
    public Category addCategory(Category category) {
        return Optional.of(category).filter(c  -> !categoryRepository.existsByName(c.getName()))
                .map(categoryRepository :: save).orElseThrow( ()->
                        new ResourceAlreadyExistesExeption("Category already exists"));
    ...
    public void deleteCategory(Long id) {
        categoryRepository.findById(id).ifPresentOrElse(categoryRepository::delete,
                () -> {
                    throw new ResourceNotFoundExeption("New resource not found");
                });
    }
